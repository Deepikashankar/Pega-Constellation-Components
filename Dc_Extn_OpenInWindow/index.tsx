...
import StyledDcExtnOpenInWindowWrapper from './styles';

// interface for props
interface DcExtnOpenInWindowProps extends PConnFieldProps {
...
function DcExtnOpenInWindow(props: DcExtnOpenInWindowProps) {
 const {
    getPConnect,
    placeholder,
    validatemessage,
  ...
  if (displayMode === 'LABELS_LEFT' || displayMode === 'DISPLAY_ONLY') {
    ...
    return displayMode === 'DISPLAY_ONLY' ? (
      <StyledDcExtnOpenInWindowWrapper> {displayComp} </StyledDcExtnOpenInWindowWrapper>
    ) : (
      <StyledDcExtnOpenInWindowWrapper>
      <FieldValueList
...
export default withConfiguration(DcExtnOpenInWindow);
